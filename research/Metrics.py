...
class Metrics(object):
    ...
    def __init__(self, prediction, y, target, threshold=4, binary=True):
        ...
        if target in ['fnat', 'irmsd', 'lrmsd']:
            ...
            # print(self.y+1,self.precision+1)
            try:
                # Mean squared logarithmic error regression loss
                self.mean_squared_log_error = metrics.mean_squared_log_error([i+1 for i in self.y], [i+1 for i in self.prediction])
            except ValueError:
                print ("WARNING: Mean Squared Logarithmic Error cannot be used when "
                            "targets contain negative values.")

            # Median absolute error regression loss
            self.median_squared_log_error = metrics.median_absolute_error([i+1 for i in self.y], [i+1 for i in self.prediction])
